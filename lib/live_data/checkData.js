          ...
          if (obj.liveData.plays) {
            ...
            if (allPlays.length > 0) {
              allPlays.forEach((play, index) => {
                ...
                if (obj.gamePk == currentGame) {
                  let eventType = play.result.eventTypeId;
                  if (eventType == "PERIOD_START") {
                    parse.sendPeriodStartMessage(play);
                  } else if (eventType == "GOAL") {
                    if (
                      obj.liveData.boxscore.teams.away.teamStats.teamSkaterStats.goals != awayGoals &&
                      obj.liveData.boxscore.teams.home.teamStats.teamSkaterStats.goals != homeGoals
                    ) {
                      parse.sendGoalMessage(obj.liveData.boxscore, play);
                      awayGoals = obj.liveData.boxscore.teams.away.teamStats.teamSkaterStats.goals;
                      homeGoals = obj.liveData.boxscore.teams.home.teamStats.teamSkaterStats.goals;
                    }
                  }
