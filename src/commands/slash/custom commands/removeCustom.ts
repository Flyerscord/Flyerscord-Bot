...
export default class RemoveCustomCommand extends AdminSlashCommand {
  constructor() {
    ...
    this.data.addStringOption((option) =>
      option.setName("name").setDescription(`The name of the command. Case insensitive`).setRequired(true)
    );
  }
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    interaction.reply({ content: `Command ${Config.getConfig().prefixes.custom}${name} removed!`, ephemeral: true });
  }
}
