...
export default (): void => {
  ...
  client.on("messageReactionAdd", async (reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser) => {
    ...
    if (!member) {
      Stumper.error(`Error finding member for user ${user.id}`, "bagReactionRole:onMessageReactionAdd");
      return;
    }
    ...
    if (reaction.emoji.id == bagEmojiId) {
      discord.roles.addRoleToUser(member, bagRoleId);
      Stumper.debug(`Reaction added to message ${reaction.message.id} by user ${user.id}`, "bagReactionRole:onMessageReactionAdd");
    }
  });
};
